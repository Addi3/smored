...
public class MarshmallowJarPerfectlyRoastedBlock extends Block {

    public static final IntProperty ITEM_COUNT = IntProperty.of("item_count", 1, 16);

    public MarshmallowJarPerfectlyRoastedBlock(Settings settings) {
        super(settings);
    ...
    public ActionResult onUse(BlockState state, World world, BlockPos pos,
        ...
        if (item == SmoredItems.MARSHMALLOW_PERFECTLY_ROASTED && currentCount < 16) {
    ...
    private ItemStack getMarshmallowOnStick(int currentCount) {
        ...
        ItemStack marshmallowOnStick = new ItemStack(SmoredItems.MARSHMALLOW_PERFECTLY_ROASTED_ON_A_STICK);
        NbtCompound nbt = marshmallowOnStick.getOrCreateNbt();
        nbt.putInt("CookLevel", 2);
        return marshmallowOnStick;
    }

    private ActionResult handleEmptyHandInteraction(World world, BlockPos pos, BlockState state,
                                                    PlayerEntity player, int currentCount) {
        if (!player.getInventory().insertStack(new ItemStack(SmoredItems.MARSHMALLOW_PERFECTLY_ROASTED))) {
            player.giveItemStack(new ItemStack(SmoredItems.MARSHMALLOW_PERFECTLY_ROASTED));
        }
    ...
    public void onBreak(World world, BlockPos pos, BlockState state, PlayerEntity player) {
        ...
        if (!player.getAbilities().creativeMode) {
            ...
            for (int i = 0; i < itemCount; i++) {
                this.dropStack(world, pos, new ItemStack(SmoredItems.MARSHMALLOW_PERFECTLY_ROASTED));
            }
        }
